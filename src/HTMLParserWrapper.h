...
static void Akeno_HTMLParser_context_getTagAttribute(const FunctionCallbackInfo<Value> &args) {
    Isolate *isolate = args.GetIsolate();
    HTMLParserWrapper *parser = getParserWrapper(args);
    ...
    if (args.Length() < 1 || !args[0]->IsString()) {
        ThrowTypeError(isolate, "Expected an attribute name");
        return;
    }

    String::Utf8Value attr(isolate, args[0]);
    std::string attrName(*attr ? *attr : "", attr.length());
    std::string value = parser->ctx.getTagAttribute(attrName);
    if (value.empty()) {
        args.GetReturnValue().Set(Null(isolate));
        return;
    }
